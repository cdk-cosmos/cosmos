import { CfnElement, IConstruct } from '@aws-cdk/core';
import { Construct, ConstructNode } from '@aws-cdk/core/lib/construct-compat';
import { Stack } from '@aws-cdk/core/lib/stack';

const PREFIX = 'COSMOS_PREFIX';
const SUFFIX = 'COSMOS_SUFFIX';
const DEFAULT_PATTERN = '{Partition}{Cosmos}[Galaxy][SolarSystem]{Resources}[Version]';
...
declare module '@aws-cdk/core/lib/construct-compat' {
  ...
  interface ConstructNode extends IPocket {}
}

ConstructNode.prototype.type = 'Resource';
// ConstructNode.prototype.pattern = '${resources}';

export class BaseStack extends Stack {
  setPrefix(value: string): void {
    this.node.setContext(PREFIX, value);
  }
  setSuffix(value: string): void {
    this.node.setContext(SUFFIX, value);
  }
  allocateLogicalId(scope: IConstruct): string {
    const scopes = scope.node.scopes.splice(1).map(x => ({
      id: x.node.id,
      type: x.node.type,
    ...
    const constructPath = scopes.map(x => x.id);
    const prefix = this.node.tryGetContext(PREFIX);
    const suffix = this.node.tryGetContext(SUFFIX);
    if (prefix) constructPath.unshift(prefix);
    if (suffix) constructPath.push(suffix);

    console.log(pattern, constructPath);

    return makeUniqueId(constructPath);
  }
}
