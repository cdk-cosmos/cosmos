...
describe('Galaxy', () => {
  ...
  test('should have cird range', () => {
    ...
    expect(galaxy.resource.networkBuilder?.addSubnet(28)).toEqual('10.0.0.0/28');
    expect(galaxy.resource.networkBuilder?.addSubnet(28)).toEqual('10.0.0.16/28');

    app = new App();
    cosmos = new CosmosCoreStack(app, 'Test', { tld: 'com' });
    galaxy = new GalaxyCoreStack(cosmos, 'Test2', { cidr: '10.0.1.0/24' });
    expect(galaxy.resource.networkBuilder?.addSubnet(28)).toEqual('10.0.1.0/28');
  });
...
describe('Galaxy Extension', () => {
  test('should be a galaxy extension', () => {
    expect(galaxyExtensionStack.name).toEqual('AppCosExtGalExtGalaxy');
  });
